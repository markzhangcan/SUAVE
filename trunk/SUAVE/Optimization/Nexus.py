...
class Nexus(Data):
    ...
    def evaluate(self, x=None):
        ...
        if (
            np.all(self.optimization_problem.inputs == self.last_inputs)
    ...
    def inequality_constraint(self, x=None):
        ...
        if not len(iqconstraints):
            ...
            constraint_evaluations[iqconstraints[:, 1] == "<"] = (
                -constraint_evaluations[iqconstraints[:, 1] == "<"]
            )
    ...
    def equality_constraint(self, x=None):
        ...
        if not len(eqconstraints):
            scaled_constraints = []
        else:
            constraint_values = help_fun.get_values(self, eqconstraints, aliases)
            scaled_constraints = help_fun.scale_const_values(
                eqconstraints, constraint_values
            ) - help_fun.scale_const_bnds(eqconstraints)
